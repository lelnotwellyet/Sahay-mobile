...
type RouteParams = {
  Chat: { sessionId: string; psychName: string };
};
...
export default function ChatScreen() {
  ...
  useEffect(() => {
    ...
    socket.on('session_ended', () => {
      Alert.alert('Session Ended', 'This session has been ended.', [
  ...
  const displayName = user?.alias ?? user?.displayName ?? 'You';
  ...
  return (
    <SafeAreaView style={styles.container}>
      ...
      <GiftedChat
        messages={messages}
        onSend={onSend}
        user={{ _id: user?.id ?? '', name: displayName }}
        isTyping={isTyping}
